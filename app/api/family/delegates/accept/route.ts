...
import { z } from "zod";
import { acceptDelegateInvite } from "@/lib/family/delegates";
...
export async function POST(req: Request) {
  ...
  try {
    const delegate = await acceptDelegateInvite(parsed.token);
    ...
    if (err instanceof Error && err.message === "INVALID_TOKEN") {
      return NextResponse.json({ error: { code: "INVALID_TOKEN", message: "Invite expired or invalid." } }, { status: 400 });
    }
    throw err;
  }
}
